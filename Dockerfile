...
EXPOSE 8080
ENV PORT=8080
CMD ["sh", "-c", "cd apps/backend && if [ ! -f /app/data/.schema-v2 ]; then rm -f /app/data/data.db /app/data/data.db-wal /app/data/data.db-shm && touch /app/data/.schema-v2; fi && npx drizzle-kit migrate && node dist/index.js"]
