...
The five demo agents (`writerbot-demo`, `reviewerbot-demo`, `consensusbot-demo`,
`summarizerbot-demo`, `observerbot-t308`) each hold an Ed25519 keypair.

### Key Generation

Keys are generated by `AgentBase.init()` on first run using `@noble/ed25519`:
...
The keypair is written as `{ sk: "<hex>", pk: "<hex>" }` at mode `0o600`.

### Key Persistence

**Local / developer runs:**
Keys live at `~/.llmtxt/demo-agents/<agentId>.key` on the developer's machine.
These files are gitignored and never committed.

**Railway deployment (`llmtxt-demo-agents` service):**
Keys persist inside the container filesystem for the lifetime of the deployment.
A redeploy or volume reset generates new keys, which are automatically
re-registered via `POST /api/v1/agents/keys` on next startup.

### Key Rotation

To rotate a demo agent key:

1. Delete the key file: `rm ~/.llmtxt/demo-agents/<agentId>.key`
2. Re-run `init()` (or restart the agent) — a new keypair is generated and
   registered automatically.
3. The old pubkey entry in `agent_pubkeys` remains (it is de-facto stale but
   harmless). To purge it, run the reset script or delete it directly:
   ```sql
   DELETE FROM agent_pubkeys WHERE agent_id = '<agentId>';
   ```

### API Key for Demo Agents

The demo agents use a shared `LLMTXT_API_KEY` env var to authenticate REST
requests. This key is stored as a Railway variable on the `llmtxt-demo-agents`
service.

**Generate a new demo API key:**
```bash
# Option A: via create-api-key script
LLMTXT_API_KEY=<admin_key> node apps/demo/scripts/create-api-key.mjs

# Option B: via admin endpoint directly
curl -s -X POST https://api.llmtxt.my/api/v1/admin/keys \
  -H "Authorization: Bearer <admin_key>" \
  -H "Content-Type: application/json" \
  -d '{"label":"demo-agents","scope":"write"}' | jq .key
```

**Set on Railway:**
```bash
railway variables --service llmtxt-demo-agents --set 'LLMTXT_API_KEY=<new_key>'
railway redeploy --service llmtxt-demo-agents --yes
```

### No Secrets Committed

The `.gitignore` in `apps/demo/` excludes `.identities/` and any `.key` files.
