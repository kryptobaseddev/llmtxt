...
import { constantTimeEqHex, hashContent, verifyContentHash } from "../wasm.js";

// Known SHA-256 vectors (verified independently via Node.js crypto)
const HELLO_HASH =
...
describe("constantTimeEqHex (S-01 / T108.7)", () => {
	...
	it("returns false when lengths differ", () => {
		assert.strictEqual(
			constantTimeEqHex(HELLO_HASH, HELLO_HASH.slice(0, 32)),
			false,
		);
	...
	it("returns false when one character differs (single-byte change)", () => {
		const modified =
			HELLO_HASH.slice(0, -1) + (HELLO_HASH.endsWith("4") ? "5" : "4");
...
describe("verifyContentHash (T-02 / T108.9)", () => {
	...
	it("round-trip: hashContent output passes verifyContentHash", () => {
		const content =
			"Round-trip: this content was hashed by hash_content and verified in constant time.";
	...
	it("round-trip: multi-paragraph content verifies correctly", () => {
		...
		assert.strictEqual(
			verifyContentHash(content.replace("\n\n", "\n"), digest),
			false,
		);
	});
	...
	it("is case-sensitive: uppercase hash does not match lowercase hash output", () => {
		...
		assert.strictEqual(
			verifyContentHash("hello", HELLO_HASH.toUpperCase()),
			false,
		);
	});
});
