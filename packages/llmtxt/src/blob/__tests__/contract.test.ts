...
describe('llmtxt/blob contract — validateBlobName', () => {
  it('accepts valid attachment names', () => {
    const valid = ['diagram.png', 'report.pdf', 'data.json', 'file-name_v2.txt', 'a'];
  ...
  it('throws BlobNameInvalidError for path traversal', () => {
    assert.throws(() => validateBlobName('../etc/passwd'), BlobNameInvalidError);
...
describe('llmtxt/blob contract — BlobFsAdapter', () => {
  ...
  it('getBlob with includeData returns correct bytes', () => {
    ...
    const result: BlobData | null = adapter.getBlob('doc-1', 'contract.txt', { includeData: true });
    assert.ok(result !== null);
  ...
  it('detachBlob returns true and hides blob from list', () => {
    ...
    adapter.attachBlob({
      docSlug: 'doc-1', name: 'detach-me.txt', contentType: 'text/plain',
      data, uploadedBy: 'agent-x',
    });
  ...
  it('LWW: re-uploading same name replaces previous record', () => {
    ...
    adapter.attachBlob({
      docSlug: 'doc-lww', name: 'item.bin', contentType: 'application/octet-stream',
      data: first, uploadedBy: 'agent-a',
    });
    adapter.attachBlob({
      docSlug: 'doc-lww', name: 'item.bin', contentType: 'application/octet-stream',
      data: second, uploadedBy: 'agent-b',
    });
...
describe('llmtxt/blob contract — error classes', () => {
  ...
  it('BlobTooLargeError: thrown when data exceeds maxBlobSizeBytes', () => {
    ...
    assert.throws(
      () => adapter.attachBlob({
        docSlug: 'doc-x', name: 'big.bin', contentType: 'application/octet-stream',
        data, uploadedBy: 'agent-a',
      }),
      BlobTooLargeError
    );
  });
  ...
  it('BlobNameInvalidError: thrown on path traversal name', () => {
    assert.throws(
      () => adapter.attachBlob({
        docSlug: 'doc-x', name: '../evil', contentType: 'text/plain',
        data: Buffer.from('x'), uploadedBy: 'agent-a',
      }),
      BlobNameInvalidError
    );
  });
  ...
  it('BlobCorruptError: thrown when file is tampered on disk', () => {
    ...
    const att = adapter.attachBlob({
      docSlug: 'doc-corrupt', name: 'tamper.txt', contentType: 'text/plain',
      data, uploadedBy: 'agent-a',
    });
...
describe('llmtxt/blob contract — incomingWinsLWW', () => {
  ...
  it('newer uploadedAt wins', () => {
    assert.equal(incomingWinsLWW(makeRef(200, 'a'), { uploadedAt: 100, uploadedBy: 'a' }), true);
  });

  it('older uploadedAt loses', () => {
    assert.equal(incomingWinsLWW(makeRef(50, 'a'), { uploadedAt: 100, uploadedBy: 'a' }), false);
  });

  it('tie-break: higher lex uploadedBy wins', () => {
    assert.equal(incomingWinsLWW(makeRef(100, 'z'), { uploadedAt: 100, uploadedBy: 'a' }), true);
    assert.equal(incomingWinsLWW(makeRef(100, 'a'), { uploadedAt: 100, uploadedBy: 'z' }), false);
  });

  it('identical record is no-op (returns false)', () => {
    assert.equal(incomingWinsLWW(makeRef(100, 'agent-a'), { uploadedAt: 100, uploadedBy: 'agent-a' }), false);
  });
});
...
describe('llmtxt/blob contract — type exports', () => {
  it('type exports are usable without import errors', () => {
    ...
    const _params: AttachBlobParams = {
      docSlug: 'doc', name: 'file.txt', contentType: 'text/plain',
      data: Buffer.from('x'), uploadedBy: 'agent',
    };
    const _ref: BlobRef = {
      blobName: 'file.txt', hash: 'a'.repeat(64),
      size: 1, contentType: 'text/plain', uploadedBy: 'agent', uploadedAt: 0,
    };
    const _cs: BlobChangeset = { crsqlChangeset: new Uint8Array(), blobs: [] };
    const _result: ApplyBlobChangesetResult = { applied: 0, discarded: 0, pendingFetches: [] };
